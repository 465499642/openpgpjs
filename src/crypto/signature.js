...
import { getPubKeyParamTypes, getPrivKeyParamTypes } from './crypto';
 ...
 * @async
 */
export async function verify(algo, hash_algo, msg_MPIs, pub_MPIs, data, hashed) {
  const types = getPubKeyParamTypes(algo);
 ...
 * @async
 */
export async function sign(algo, hash_algo, key_params, data, hashed) {
