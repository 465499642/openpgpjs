 ...
 * @async
 */
export async function generate(B, E) {
  let key;
  E = new BN(E, 16);
  ...
  let q = await randomProbablePrime(B - (B >> 1), E, 40);
  let p = await randomProbablePrime(B >> 1, E, 40);
...
async function bnDecrypt(data, n, e, d, p, q, u) {
  ...
  if (config.rsaBlinding) {
    unblinder = (await getRandomBN(new BN(2), n)).toRed(nred);
  ...
  return emeDecode((new type_mpi(result)).toString());
}
