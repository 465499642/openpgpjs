 ...
 * @async
 */
export async function publicKeyEncrypt(algo, pub_params, data, fingerprint) {
  ...
  switch (algo) {
    ...
    case enums.publicKey.elgamal: {
      data = new type_mpi(await emeEncode(data, pub_params[0].byteLength()));
      const m = data.toBN();
 ...
 * @async
 */
export async function publicKeyDecrypt(algo, key_params, data_params, fingerprint) {
  switch (algo) {
    case enums.publicKey.rsaEncryptSign:
    case enums.publicKey.rsaEncrypt: {
    ...
    case enums.publicKey.elgamal: {
      ...
      return emeDecode(result.toString());
    }
    case enums.publicKey.ecdh: {
 ...
 */
export function getPrivKeyParamTypes(algo) {
  switch (algo) {
 ...
 * @returns {Array<String>} The array of types
 */
export function getPubKeyParamTypes(algo) {
  switch (algo) {
...
export function generateParams(algo, bits, oid) {
  const types = [].concat(getPubKeyParamTypes(algo), getPrivKeyParamTypes(algo));
  switch (algo) {
    case enums.publicKey.rsaEncrypt:
    case enums.publicKey.rsaEncryptSign:
    case enums.publicKey.rsaSign: {
